...
- **Implementation:** `MiniVM` now derives the `CALL` target via PC-relative math using word offsets masked to 16 bits, the assembler/linker emit matching offsets, and regression tests cover backward calls plus relocation math (`PYTHONPATH=. pytest python/tests/test_vm_callret.py python/tests/test_asm_local_relocs.py python/tests/test_reloc_patch_unit.py`). Demos link cleanly via `make -C examples/demos`.
