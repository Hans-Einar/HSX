...
- `disassemble`: Render MVASM listings when source is unavailable

#### 5.2.1 Disassembly Panel
- The adapter issues `disasm.read` (falling back to legacy `disasm`) with `view: "around_pc"` whenever the Run/Debug Disassembly view follows the active PC. Manual address requests keep using `view: "from_addr"`.
- Executives must expose immutable code bytes (see docs/hxe_format.md); adapters highlight the `referenceAddress` the server returns instead of guessing based on the request offset.
- Telemetry: every disassembly request logs `{view, count, reference}` so VS Code can surface warnings when an executive is outdated (i.e., falls back to legacy RPCs or returns zeroed instructions). Troubleshooting guidance in the Implementation Plan and docs/hsx_dbg_usage.md walks users through interpreting these events.
