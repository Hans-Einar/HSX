...
- **Input:** LLVM IR (.ll or .bc files) from clang or other front-ends.
- **Output:** MVASM source (.asm) with register assignments and relocations.
- **Processing:**
  1. Legalize LLVM IR (expand unsupported ops, lower intrinsics).
  2. Perform register allocation (linear scan, caller/callee-saved conventions).
  3. Lower arithmetic, control flow, and SVC calls to MVASM opcodes.
  4. Emit prologue/epilogue sequences per ABI.
  5. Generate relocation entries for symbols and external references.

#### 4.2.2 Assembly (`asm.py`)
- **Input:** MVASM source with directives (`.text`, `.data`, `.bss`, labels, symbols).
- **Output:** HXO object file (JSON header + binary sections).
- **Processing:**
  1. Parse MVASM syntax and resolve local labels.
  2. Encode opcodes and operands per [docs/MVASM_SPEC.md](../../docs/MVASM_SPEC.md).
  3. Build relocation table for unresolved external symbols.
  4. Emit symbol table with section offsets.
  5. Optionally generate listing file and JSON debug metadata (`--dump-json`).
...
- **Output:** HXE executable with header, code/data sections, and embedded manifest.
- **Processing:**
  1. Merge sections from all HXO inputs (.text, .data, .bss).
  2. Resolve external symbol references and apply relocations.
  3. Allocate final addresses and compute entry point.
  4. Generate HXE header with magic, version, flags (including allow_multiple_instances), app_name, section sizes, CRC32.
  5. Optionally embed provisioning manifest (FRAM keys, values) per [docs/hxe_format.md](../../docs/hxe_format.md).
  6. Emit combined symbol/line info for debugger.

**App Name Handling:**
- Linker accepts `--app-name <name>` parameter (required).
...
- CRC calculation excludes app_name field to allow post-link name updates if needed.
...
| `asm.py` | MVASM (.asm) | HXO (.hxo) | `--emit-hxo`, `--dump-json`, `--listing` |
| `hld.py` | HXO files (.hxo) | HXE (.hxe) | `--output`, `--manifest`, `--strip-debug` |
