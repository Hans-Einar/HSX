...
#### 5.3.1 Attach to Executive (global)
```json
Request: {
  "version": 1,
  "cmd": "attach",
...
Response: {
  ...
  "info": {
    ...
    "tasks": {
      "current_pid": 1,
      "tasks": [
        {"pid": 1, "state": "paused", "app_name": "longrun/main", "...": "..."}
      ]
    }
  }
}
...
> **Note:** The current executive exposes a global attach that pauses the VM and grants the session exclusive control. Per-PID locking/observer modes remain on the roadmap; once supported, the request will accept a `pid` field matching the CLI `attach <pid>` UX.

#### 5.3.2 Detach from Executive
```json
Request: {
  "version": 1,
  "cmd": "detach",
...
|---------|-----------|-------------|
| `connect` | `[--host <addr>] [--port <n>]` | Establish session with executive (defaults 127.0.0.1:9998) |
| `status` | - | Show connection status, negotiated capabilities, session id |
| `attach` | - | Pause VM and take global ownership (future: `attach <pid>` when per-PID locks land) |
| `detach` | - | Release global ownership / resume VM |
...
| `info` | `[pid]` | Show global scheduler info or per-task details/registers |
| `help` | `[command]` | Show help for command |
| `quit` / `exit` | - | Close session and exit |

> Task listings (e.g., `ps`, `info`) display the resolved `app_name` and metadata counts (`values`, `commands`, `mailboxes`) when the executive surfaces those summaries (Executive Phase 3.3).
