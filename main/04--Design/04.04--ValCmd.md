...
| `reserved` | uint8 | 8 | Reserved for future use. |

OID (Object ID) = `(group_id << 8) | value_id` provides unique 16-bit identifier.

#### 4.2.2 Value Descriptor (`hsx_val_desc`)
Optional metadata structure for tooling and rich display.

| Field | Type | Description |
|-------|------|-------------|
| `group_name` | string ptr | Null-terminated group name (deduplicated in string table). |
...
Descriptors are optional; omit to save RAM on constrained targets.

#### 4.2.3 Command Entry (`hsx_cmd_entry`)
...
| `reserved` | uint32 | 32 | Reserved for future arguments/return types. |

#### 4.2.4 Command Descriptor (`hsx_cmd_desc`)
Optional metadata for shell/debugger display.

| Field | Type | Description |
|-------|------|-------------|
| `group_name` | string ptr | Group name (shared with values). |
...
4. Optionally copy descriptor strings into shared string table (deduplicated).
5. Assign persist key if provided; auto-allocate from FRAM pool if zero and Persist flag set.
6. Emit `value_registered` or `cmd_registered` event for tooling.
