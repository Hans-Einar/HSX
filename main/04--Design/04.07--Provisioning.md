...
- **Source selection and transport:** choose and drive a HAL binding (FS, CAN, UART, …).
- **Image verification:** header/section checks, optional signature and capability checks, CRC.
- **Coordination with VM:** call `vm_load_begin/write/end/abort` (streaming) or `vm_load_hxe` (monolithic); call `vm_set_context` as needed; never force VM to auto-switch.
