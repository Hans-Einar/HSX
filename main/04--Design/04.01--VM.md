...
|--------------|----------|---------------------|
| 0x01 | LDI | Rd:GPR32, Imm12:signed literal or reloc |
...
## 8. HAL Integration and Shims
- When the executive is absent, SVC handlers delegate to HAL modules (`04.08--HAL.md`) for UART, timers, persistent storage, and related services.
- Attached mode defers hardware access to the executive, which uses the same HAL for drivers to keep a single source of truth.
- HAL capability negotiation lets the VM discover which services are local versus executive-provided; unsupported services trigger fallback handling or explicit errors.
...
- Syscall coverage: confirm return codes, error propagation, and HAL shims behave consistently in standalone and attached modes.
...
- Standalone stress: run repetitive workloads without an executive to validate HAL shims and timing behaviour.
