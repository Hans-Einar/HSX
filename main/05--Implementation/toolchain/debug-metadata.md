...
*Current implementation*: Phase 3.3 emits `functions`/`files` along with fully populated `line_map` (including MVASM ordinals) and `llvm_to_mvasm` (line + ordinal coverage) via `hsx-llc --emit-debug`. Variable metadata remains pending for Phase 3.4.
