...
*Current implementation*: Phase 3.3 emits `functions`/`files` along with populated `line_map` and `llvm_to_mvasm` entries via `hsx-llc --emit-debug`. Variable metadata remains pending for Phase 3.4.
