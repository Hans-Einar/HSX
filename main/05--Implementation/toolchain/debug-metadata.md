...
*Current implementation*: Phase 3.2 emits only the `functions`/`files` portions of the schema (including `mvasm_start_line` / `mvasm_end_line`) via `hsx-llc --emit-debug`. The `line_map`, `variables`, and `llvm_to_mvasm` sections will be populated in Phase 3.3/3.4.
