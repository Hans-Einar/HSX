...
- `line_map[].source_kind` identifies whether the instruction came from user source (`"source"`) or was injected by the compiler (`"compiler"`). Compiler-tagged instructions omit `source_line` but still carry ordinals and instruction ids so debuggers can offer “step over” semantics.
- `line_map[].instruction` references the internal instruction id (`function@N`), enabling downstream tools to correlate MVASM lines back to LLVM IR even when no source line exists.
- `line_coverage` summarises instruction-to-line coverage:

```json
{
  "line_coverage": {
    "total_instructions": 128,
    "source_mapped": 120,
    "compiler_tagged": 8,
    "unmapped": []
  }
...
If `unmapped` is non-empty, hsx-llc emits a warning (and the entries remain absent from `line_map`), allowing CI to flag incomplete debug coverage.
