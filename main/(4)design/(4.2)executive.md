...
## Data Structures
- **ExecutiveState:**
  - `tasks: Dict[int, TaskRecord]` (PID â†’ metadata).
...
- **Event queue:** `self._event_bus` (future enhancement) storing `Event{seq, ts, type, payload}`; current implementation uses `log_buffer` + event fetch.
...
- **Event streaming:**
  - Collect events from `MiniVM.consume_events()` (debug_stop, svc, mailbox events) + executive instrumentation (scheduler trace, provisioning logs).
  - Each subscriber maintains `last_seq`; event fetch API returns `{events, next_seq}` with optional filters (pid, type).
  - Back-pressure: bounded queue (default 512 entries). When overflow, drop oldest and emit `event_dropped` warning.
- **Provisioning flow:**
...
- Scheduler metrics (`scheduler_trace`, `scheduler_counters`) log transitions (`READYâ†’RUNNING`, `RUNNINGâ†’WAIT_MBX`, etc.) so tooling can detect starvation or imbalance.
