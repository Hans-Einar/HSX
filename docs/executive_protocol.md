...
| `trace` | `{ "version": 1, "cmd": "trace", "pid": 1, "mode": "on" }`<br>`{ "version": 1, "cmd": "trace", "pid": 1, "op": "records", "limit": 32 }` | `{ "version": 1, "status": "ok", "trace": { "pid": 1, "enabled": true, "buffer_size": 256 } }`<br>`{ "version": 1, "status": "ok", "trace": { "pid": 1, "capacity": 256, "count": 64, "returned": 32, "records": [ { "seq": 17, "pc": 4096, "opcode": 57005, "ts": 1730512345.12, "changed_regs": ["R0"] }, ... ] } }` | Enable/disable instruction tracing or fetch the most recent trace records for a task. When no `mode` is supplied the executive toggles the existing state; `op: "records"` returns the per-task ring buffer (optionally limited via `limit`). |
