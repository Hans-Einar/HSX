...
| `val.set` | `{ "version": 1, "cmd": "val.set", "oid": 0x0201, "value": 15.0 [, "pid": 1] }` | `{ "version": 1, "status": "ok", "value": { "status": 0, "value": 15.0, "f16": 16896, "pid": 1 } }` | Writes a new value through the registry (emits `value_changed` events and notifies subscribers). PID defaults to the owner. Returns `status=EBUSY` when the value is throttled by its configured `rate_ms`. |
| `val.stats` | `{ "version": 1, "cmd": "val.stats" }` | `{ "version": 1, "status": "ok", "stats": { "values": { "count": 5, "capacity": 32, "usage_pct": 15.6, "high_water": 7, "high_water_pct": 21.8, "warning": false }, "strings": { "used_bytes": 480, "total_bytes": 4096, "usage_pct": 11.7, "high_water_bytes": 640, "high_water_pct": 15.6, "warning": false } } }` | Returns current occupancy metrics for the value table and shared string pool. Tooling can poll this to watch for exhaustion before provisioning additional values. |
...
| `cmd.call` | `{ "version": 1, "cmd": "cmd.call", "oid": 0x0304 [, "pid": 1, "token": "pin-1234", "async": true] }` | `{ "version": 1, "status": "ok", "command": { "status": 0, "result": "ok", "pid": 1 } }` | Executes a command handler. Provide `token` for commands flagged with PIN auth; set `async:true` to have the result delivered via mailbox. |
| `cmd.stats` | `{ "version": 1, "cmd": "cmd.stats" }` | `{ "version": 1, "status": "ok", "stats": { "commands": { "count": 4, "capacity": 32, "usage_pct": 12.5, "high_water": 6, "high_water_pct": 18.8, "warning": false }, "strings": { "used_bytes": 512, "total_bytes": 4096, "usage_pct": 12.5, "high_water_bytes": 640, "high_water_pct": 15.6, "warning": false } } }` | Mirrors `val.stats` for the command table; the shared string statistics appear in both responses. |
