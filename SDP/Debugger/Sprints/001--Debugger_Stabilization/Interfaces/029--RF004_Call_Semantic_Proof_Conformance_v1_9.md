...
2. `byte_size == 4` is mandatory, otherwise `CORRUPT / instruction_encoding_contract`;
...
6. every other fixed32 opcode value is proven non-CALL;
7. any other instruction encoding yields `UNSUPPORTED / instruction_encoding_unsupported`.

The local fixed32 CALL constant is permitted only to keep the pure debugger package
self-contained. Conformance tests must assert that it equals `opcodes.OPCODES["CALL"]` and that
the decode rule matches the existing disassembler's primary-opcode extraction.
...
- proven CALL -> publish `call_site_pc=candidate` and use that candidate as the next caller
  `UnwindFrame.pc`;
- proven non-CALL -> terminate `CORRUPT / call_site_not_call` with the existing trustworthy
  prefix preserved under 1.6;
