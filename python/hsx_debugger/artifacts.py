...
def _address(value: object, field_name: str) -> HsxAddress:
    item = _record(
        value,
        frozenset({"space_id", "unsigned_value"}),
        frozenset(),
        field_name,
    )
    return HsxAddress(
...
def _abi(value: object, field_name: str) -> AbiDescriptorRef:
    ...
    return AbiDescriptorRef(
        _nonempty(item["ref"], f"{field_name}.ref"),
        _nonempty(item["digest"], f"{field_name}.digest"),
    )
...
def _source_location(
    ...
    item = _record(
        value,
        frozenset({"logical_id", "line"}),
        frozenset({"column", "discriminator"}),
        field_name,
    )
    ...
    if source is None:
        raise _fail(
            ResolutionStatus.CORRUPT,
            "source_identity_join_missing",
            f"{field_name} names unknown source logical ID {logical_id!r}",
            component="symbol_model",
        )
    return SourceLocation(
        source,
        _uint(item["line"], f"{field_name}.line", minimum=1),
        _uint(item["column"], f"{field_name}.column", minimum=1)
        if "column" in item
        else None,
        _uint(item["discriminator"], f"{field_name}.discriminator")
...
def _type_member(value: object) -> TypeMember:
    item = _record(
        value,
        frozenset({"name", "type_id", "bit_offset", "bit_size"}),
        frozenset(),
        "type member",
    )
    return TypeMember(
...
def _parse_symbol_records(
    ...
    document = _record(
        payload,
        frozenset({"component_schema", "records"}),
        frozenset(),
        "symbol component",
    )
    if document["component_schema"] != _SYMBOL_SCHEMA:
        raise _fail(
            ...
            "symbol component_schema is not hsx.debug-component.symbol-model/1",
            component="symbol_model",
        )
    ...
    for raw in _sequence(document["records"], "symbol component records"):
        if not isinstance(raw, Mapping):
            raise _fail(
                ResolutionStatus.CORRUPT,
                "malformed_component_record",
                "symbol component records must be objects",
                component="symbol_model",
            )
        ...
        if not isinstance(record_kind, str):
            raise _fail(
                ResolutionStatus.CORRUPT,
                "missing_component_record_kind",
                "symbol component record_kind must be a string",
                component="symbol_model",
            )
        if record_kind == "function":
            item = _record(
                raw,
                frozenset({"record_kind", "function_id", "name", "range"}),
                frozenset({"linkage_name", "definition"}),
                "function record",
            )
            functions.append(
                FunctionRecord(
                    ...
                    _nonempty(item["linkage_name"], "function.linkage_name")
                    if "linkage_name" in item
                    else None,
                    _address_range(item["range"], "function.range"),
                    _source_location(item["definition"], source_refs, "function.definition")
            ...
            item = _record(
                ...
                frozenset({"address", "function_id", "lexical_scope_id", "type_id"}),
                "symbol record",
            )
            symbols.append(
                SymbolRecord(
                    ...
                    _nonempty(item["function_id"], "symbol.function_id")
                    if "function_id" in item
                    else None,
                    _nonempty(item["lexical_scope_id"], "symbol.lexical_scope_id")
                    if "lexical_scope_id" in item
                    else None,
            ...
            item = _record(
                raw,
                frozenset(
                    {"record_kind", "type_id", "name", "kind", "bit_size", "byte_order", "members"}
                ),
                frozenset(),
                "type record",
            )
            ...
            item = _record(
                ...
                frozenset({"parent_scope_id"}),
                "lexical scope record",
            )
            scopes.append(
                LexicalScopeRecord(
                    _nonempty(item["lexical_scope_id"], "lexical_scope_id"),
                    _nonempty(item["function_id"], "scope.function_id"),
                    _nonempty(item["parent_scope_id"], "scope.parent_scope_id")
                    if "parent_scope_id" in item
                    else None,
            ...
            item = _record(
                ...
                frozenset({"encoded_word", "function_id", "source"}),
                "instruction record",
            )
            instructions.append(
                InstructionRecord(
                    ...
                    _uint(item["encoded_word"], "instruction.encoded_word")
                    if "encoded_word" in item
                    else None,
                    _nonempty(item["function_id"], "instruction.function_id")
                    if "function_id" in item
                    else None,
                    _source_location(item["source"], source_refs, "instruction.source")
                    if "source" in item
                    else None,
                    _enum(
                        InstructionClassification,
            ...
            item = _record(
                raw,
                frozenset(
                    {"record_kind", "region_id", "name", "kind", "range", "permissions"}
                ),
                frozenset(),
                "memory region record",
            )
            permissions = frozenset(
                _enum(Permission, permission, "memory_region.permission")
                for permission in _sequence(item["permissions"], "memory_region.permissions")
            )
            regions.append(
                MemoryRegion(
                    ...
                    _address_range(item["range"], "memory_region.range"),
                    permissions,
                )
            )
        else:
            raise _fail(
                ResolutionStatus.SCHEMA_UNSUPPORTED,
                "unsupported_component_record_kind",
                f"unsupported symbol record_kind {record_kind!r}",
                component="symbol_model",
            )
    return (
        tuple(functions),
...
def _opcode_wire(value: object) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise _fail(
            ...
            "recipe opcode must be an object",
            component="recipe",
        )
    ...
    shapes: dict[str, tuple[frozenset[str], frozenset[str]]] = {
        ...
        "bit_slice": (
            frozenset({"opcode", "source_bit_offset", "bit_size"}),
            frozenset(),
        ),
    }
    if not isinstance(opcode, str):
        raise _fail(
            ResolutionStatus.CORRUPT,
            "malformed_recipe_opcode",
            "recipe opcode name must be a string",
            component="recipe",
        )
...
def _expression_wire(value: object) -> Mapping[str, object]:
    item = _record(
        value,
        frozenset({"role", "opcodes", "required_result"}),
        frozenset({"required_bit_width"}),
        "recipe expression",
    )
    return {
        "role": item["role"],
        ...
        "required_bit_width": _uint(item["required_bit_width"], "required_bit_width", minimum=1)
        if "required_bit_width" in item
        else None,
    }
...
def _piece_wire(value: object) -> Mapping[str, object]:
    item = _record(
        value,
        frozenset({"destination_bit_offset", "bit_size", "expression", "source_bit_offset"}),
        frozenset(),
        "location piece",
    )
    return {
...
def _location_form_wire(value: object) -> Mapping[str, object]:
    item = _record(
        value,
        frozenset({"kind"}),
        frozenset({"expression", "pieces", "reason"}),
        "location form",
    )
    return {
        "kind": item["kind"],
...
def _parse_unwind_records(
    ...
    if document["component_schema"] != _UNWIND_SCHEMA:
        raise _fail(
            ResolutionStatus.SCHEMA_UNSUPPORTED,
            "unsupported_component_schema",
            "unwind component_schema is unsupported",
    ...
    for raw in _sequence(document["records"], "unwind records"):
        item = _record(
            ...
            frozenset({"caller_frame_base_rule", "call_site_adjustment"}),
            "unwind row",
        )
        ...
        if schema != _UNWIND_SCHEMA:
            raise _fail(
                ...
                f"unwind row schema {schema!r} is unsupported",
                component="unwind_recipe",
            )
        ...
        for entry in _sequence(item["register_rules"], "register_rules"):
            if not isinstance(entry, list) or len(entry) != 2:
                raise _fail(
                    ...
                    "register_rules entries must be two-element arrays",
                    component="unwind_recipe",
                )
            register_rules.append(
                (_nonempty(entry[0], "register rule ID"), _rule_wire(entry[1]))
            )
        parser_input = {
            ...
            "caller_frame_base_rule": _rule_wire(item["caller_frame_base_rule"])
            if "caller_frame_base_rule" in item
            else None,
            "register_rules": tuple(register_rules),
            "boundary": item["boundary"],
            "call_site_adjustment": _sint(item["call_site_adjustment"], "call_site_adjustment")
            if "call_site_adjustment" in item
            else None,
        }
...
def _parse_location_records(
    ...
    if document["component_schema"] != _LOCATION_SCHEMA:
        raise _fail(
            ResolutionStatus.SCHEMA_UNSUPPORTED,
            "unsupported_component_schema",
            "location component_schema is unsupported",
    ...
    for raw in _sequence(document["records"], "location records"):
        item = _record(
            ...
            frozenset({"lexical_scope_id", "function_id", "declared_type_id"}),
            "location row",
        )
        ...
        if schema != _LOCATION_SCHEMA:
            raise _fail(
                ResolutionStatus.SCHEMA_UNSUPPORTED,
                "unsupported_recipe_schema",
                f"location row schema {schema!r} is unsupported",
                component="location_recipe",
            )
        parser_input = {
            ...
            "lexical_scope_id": _nonempty(item["lexical_scope_id"], "location.lexical_scope_id")
            if "lexical_scope_id" in item
            else None,
            "function_id": _nonempty(item["function_id"], "location.function_id")
            if "function_id" in item
            else None,
            "pc_range": _address_range(item["pc_range"], "location.pc_range"),
            "declared_type_id": _nonempty(item["declared_type_id"], "location.declared_type_id")
            if "declared_type_id" in item
            else None,
            "declared_bit_size": _uint(
                item["declared_bit_size"], "location.declared_bit_size", minimum=1
            ),
...
def _range_overlaps(left: HsxAddressRange, right: HsxAddressRange) -> bool:
    return (
        left.start.space == right.start.space
        and left.start.unsigned_value < right.end_unsigned_value
        and right.start.unsigned_value < left.end_unsigned_value
    )
...
def _validate_range(
    ...
    *,
    ...
    if pc_space and value.start.space != architecture.pc_space:
        raise _fail(
            ResolutionStatus.CORRUPT,
            "record_pc_space_mismatch",
            f"{label} is not in architecture.pc_space",
            component="artifact",
        )
    ...
    if result.status is not AddressStatus.VALID:
        raise _fail(
            ...
            f"{label}: {result.diagnostics[0].message}",
            component="artifact",
        )


def _validate_portable_records(
    architecture: ArchitectureDescriptor,
    ...
    for function in functions:
        _validate_range(
            architecture,
            function.range,
            f"function {function.function_id!r}",
        ...
        if function.definition is not None and function.definition.source not in source_set:
            raise _fail(
                ResolutionStatus.CORRUPT,
                "source_identity_join_missing",
                "function definition SourceRef is outside the accepted manifest",
                component="symbol_model",
            )
    ...
    for prior, current in zip(ordered_functions, ordered_functions[1:]):
        if _range_overlaps(prior.range, current.range) and not _same_range(prior.range, current.range):
            raise _fail(
                ResolutionStatus.CORRUPT,
                "overlapping_function_ranges",
                "function ranges overlap without being exact aliases",
                component="symbol_model",
            )
    ...
    for record in types:
        for member in record.members:
            if member.type_id not in type_by_id:
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "type_member_join_missing",
                    f"type member names unknown type {member.type_id!r}",
                    component="symbol_model",
                )
    ...
    for scope in scopes:
        ...
        if function is None:
            raise _fail(
                ResolutionStatus.CORRUPT,
                "scope_function_join_missing",
                f"scope names unknown function {scope.function_id!r}",
                component="symbol_model",
            )
        _validate_range(
            architecture,
            scope.pc_range,
            f"scope {scope.lexical_scope_id!r}",
            permission=Permission.EXECUTE,
            pc_space=True,
        )
        if not function.range.contains_range(scope.pc_range):
            raise _fail(
                ResolutionStatus.CORRUPT,
                "scope_outside_function",
                "lexical scope range must be contained by its function",
                component="symbol_model",
            )
        if scope.parent_scope_id is not None:
            ...
            if parent is None or parent.function_id != scope.function_id:
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "scope_parent_join_missing",
                    "lexical scope parent must exist in the same function",
                    component="symbol_model",
                )
            if not parent.pc_range.contains_range(scope.pc_range):
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "scope_parent_range_mismatch",
                    "lexical scope must be contained by its parent",
                    component="symbol_model",
                )
        visited: set[str] = set()
        current = scope
        while current.parent_scope_id is not None:
            if current.lexical_scope_id in visited:
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "lexical_scope_cycle",
                    "lexical scope parent graph contains a cycle",
                    component="symbol_model",
                )
    ...
    for symbol in symbols:
        if symbol.type_id is not None and symbol.type_id not in type_by_id:
            raise _fail(
                ...
                f"symbol names unknown type {symbol.type_id!r}",
                component="symbol_model",
            )
        if symbol.kind is SymbolKind.FUNCTION:
            ...
            if function is None or symbol.address != function.range.start:
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "function_symbol_join_mismatch",
                    "FUNCTION symbol must name and start at its exact FunctionRecord",
                    component="symbol_model",
                )
            ...
            if symbol.address.space != architecture.pc_space or checked.status is not AddressStatus.VALID:
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "invalid_label_address",
                    "LABEL symbol address must be valid executable pc-space evidence",
                    component="symbol_model",
                )
            if symbol.function_id is not None:
                ...
                if function is None or not function.range.contains(symbol.address):
                    raise _fail(
                        ResolutionStatus.CORRUPT,
                        "label_function_join_mismatch",
                        "LABEL function must exist and contain its address",
                        component="symbol_model",
                    )
            if symbol.lexical_scope_id is not None:
                ...
                if (
                    scope is None
                    or scope.function_id != symbol.function_id
                    or not scope.pc_range.contains(symbol.address)
                ):
                    raise _fail(
                        ResolutionStatus.CORRUPT,
                        "label_scope_join_mismatch",
                        "scoped LABEL must resolve inside its exact function/scope",
                        component="symbol_model",
                    )
            ...
            if function is None or scope is None or scope.function_id != function.function_id:
                raise _fail(
                    ...
                    "scoped variable must resolve to its exact function and lexical scope",
                    component="symbol_model",
                )
    ...
    for instruction in instructions:
        ...
        if instruction.address.space != architecture.pc_space or checked.status is not AddressStatus.VALID:
            raise _fail(
                ...
                "instruction address must be valid executable pc-space evidence",
                component="symbol_model",
            )
        ...
        if range_result.status is not AddressStatus.VALID:
            raise _fail(
                ResolutionStatus.CORRUPT,
                "invalid_instruction_range",
                range_result.diagnostics[0].message,
                component="symbol_model",
            )
        if instruction.function_id is not None:
            ...
            if function is None or not function.range.contains_range(range_result.value):
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "instruction_function_join_mismatch",
                    "instruction function must exist and contain the instruction",
                    component="symbol_model",
                )
        if instruction.source is not None and instruction.source.source not in source_set:
            raise _fail(
                ResolutionStatus.CORRUPT,
                "source_identity_join_missing",
                "instruction SourceRef is outside the accepted manifest",
                component="symbol_model",
            )
    ...
    instruction_ranges.sort(
        key=lambda item: (
            item[1].start.space.value,
            item[1].start.unsigned_value,
            item[0].instruction_id,
        )
    )
    for (prior_record, prior), (current_record, current) in zip(
        instruction_ranges, instruction_ranges[1:]
    ):
        if _range_overlaps(prior, current) and not _same_range(prior, current):
            raise _fail(
                ResolutionStatus.CORRUPT,
                "overlapping_instruction_ranges",
                f"instructions {prior_record.instruction_id!r} and {current_record.instruction_id!r} overlap",
                component="symbol_model",
            )
    ...
    for region in ordered_regions:
        ...
        if descriptor is None or not region.permissions.issubset(descriptor.permissions):
            raise _fail(
                ResolutionStatus.CORRUPT,
                "memory_region_permission_mismatch",
                "memory region permissions exceed its address-space descriptor",
                component="symbol_model",
            )
    ...
    ordered_unwind = sorted(
        unwind_rows,
        key=lambda row: (
            row.pc_range.start.space.value,
            row.pc_range.start.unsigned_value,
            row.row_id,
        ),
    )
    for row in ordered_unwind:
        if row.abi != abi:
            raise _fail(
                ResolutionStatus.CORRUPT,
                "row_abi_mismatch",
                "unwind row ABI differs from the validated accepted ABI",
                component="unwind_recipe",
                row_id=row.row_id,
            )
    ...
    for prior, current in zip(ordered_unwind, ordered_unwind[1:]):
        if _range_overlaps(prior.pc_range, current.pc_range):
            raise _fail(
                ResolutionStatus.CORRUPT,
                "overlapping_unwind_rows",
                "unwind rows must not overlap",
                component="unwind_recipe",
            )
    ...
    for row in location_rows:
        if row.abi != abi:
            raise _fail(
                ResolutionStatus.CORRUPT,
                "row_abi_mismatch",
                "location row ABI differs from the validated accepted ABI",
                component="location_recipe",
                row_id=row.row_id,
            )
        ...
        if variable is None:
            raise _fail(
                ResolutionStatus.CORRUPT,
                "location_symbol_join_missing",
                f"location row names unknown symbol {row.symbol_id!r}",
                component="location_recipe",
                row_id=row.row_id,
            )
        diagnostics = RecipeComponentValidator.validate_location_row(
        ...
        if row.declared_type_id is not None:
            ...
            if (
                ...
                or declared_type.bit_size != row.declared_bit_size
                or declared_type.byte_order != row.value_byte_order
            ):
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "location_type_join_mismatch",
                    "location row type width/byte order differs from its exact TypeRecord",
                    component="location_recipe",
                    row_id=row.row_id,
                )
    ...
    for rows in location_groups.values():
        ...
        for prior, current in zip(rows, rows[1:]):
            if _range_overlaps(prior.pc_range, current.pc_range):
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "overlapping_location_rows",
                    "location rows for one exact variable identity must not overlap",
                    component="location_recipe",
                )
    ...
    for symbol in symbols:
        if symbol.kind in {SymbolKind.GLOBAL, SymbolKind.LOCAL, SymbolKind.CONSTANT}:
            if symbol.symbol_id not in location_symbols:
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "variable_location_join_missing",
                    f"variable {symbol.symbol_id!r} has no LocationRow",
                    component="location_recipe",
                )
        elif symbol.symbol_id in location_symbols:
            raise _fail(
                ResolutionStatus.CORRUPT,
                "address_symbol_has_location_row",
                "FUNCTION/LABEL symbols must not have LocationRows",
                component="location_recipe",
            )
...
def _query_result(
    ...
    values: tuple[object, ...],
    *,
    multiple: ResolutionStatus = ResolutionStatus.CORRUPT,
    ...
    if not values:
        return ResolutionResult(
            ...
            binding,
            (),
            (Diagnostic(unavailable_code, "no exact artifact record matched", component="artifact_index"),),
        )
    if len(values) == 1:
    ...
    return ResolutionResult(
        multiple,
        binding,
        values if multiple is ResolutionStatus.AMBIGUOUS else (),
        (Diagnostic(multiple_code, "more than one exact artifact record matched", component="artifact_index"),),
    )
...
class DebugArtifactIndex:
    ...
    def build(
        ...
        try:
            if (
                bundle_identity.source_identity_manifest_ref.algorithm != "sha256"
                or source_manifest.canonical_digest()
                != bundle_identity.source_identity_manifest_ref.value
            ):
                raise _fail(
                    ResolutionStatus.ARTIFACT_MISMATCH,
                    "source_manifest_digest_mismatch",
                    "SourceIdentityManifest digest differs from the accepted bundle",
                    component="source_identity_manifest",
                )
            ...
            unknown_schemas = tuple(
                ...
                if component.schema not in _COMPONENT_DOMAINS
            )
            if unknown_schemas:
                raise _fail(
                    ResolutionStatus.SCHEMA_UNSUPPORTED,
                    "unsupported_component_schema",
                    f"unsupported component schemas {unknown_schemas!r}",
                    component="component",
                )
            ...
            if missing or duplicated:
                raise _fail(
                    ResolutionStatus.CORRUPT,
                    "component_cardinality_mismatch",
                    f"mandatory component cardinality mismatch; missing={missing!r}, duplicated={duplicated!r}",
                    component="component",
                )
            ...
            for schema, items in by_schema.items():
                ...
                if component.canonical_digest != expected_digest:
                    raise _fail(
                        ...
                        f"component {component.component_id!r} digest differs from the bundle ref",
                        component=component.component_id,
                    )
                ...
                if recomputed != component.canonical_digest:
                    raise _fail(
                        ...
                        f"component {component.component_id!r} content digest differs",
                        component=component.component_id,
                    )
            ...
            source_refs = tuple(
                SourceRef(bundle_ref, record.logical_id, record.content_digest, record.byte_length)
            ...
            (
                ...
                instructions,
                regions,
            ) = _parse_symbol_records(payloads[_SYMBOL_SCHEMA], source_by_id)
            unwind_rows = _parse_unwind_records(
                payloads[_UNWIND_SCHEMA], binding, expected_digests[_UNWIND_SCHEMA]
            )
            location_rows = _parse_location_records(
                payloads[_LOCATION_SCHEMA], binding, expected_digests[_LOCATION_SCHEMA]
            )
            _validate_portable_records(
                architecture,
                abi,
                source_refs,
                functions,
            ...
            index = cls._from_validated(
                ...
                tuple(
                    sorted(
                        symbols,
                        key=lambda record: (_SYMBOL_KIND_RANK[record.kind], record.symbol_id),
                    )
                ),
                ...
                tuple(
                    sorted(
                        scopes,
                        key=lambda record: (record.declaration_order, record.lexical_scope_id),
                    )
                ),
                tuple(
                    sorted(
            ...
            status = (
                ResolutionStatus.SCHEMA_UNSUPPORTED
                if exc.status is RecipeEvaluationStatus.UNSUPPORTED
                else ResolutionStatus.CORRUPT
            )
            ...
            return ResolutionResult(
                ...
                binding,
                (),
                (Diagnostic("malformed_component", str(exc), component="artifact"),),
            )
    ...
    def type_by_id(self, type_id: str) -> ResolutionResult[TypeRecord]:
        ...
        return _query_result(
            self._binding,
            values,
            unavailable_code="type_unavailable",
            multiple_code="duplicate_type_identity",
        )

    def lexical_scopes(
        self, function_id: str, frame_pc: HsxAddress
    ) -> tuple[LexicalScopeRecord, ...]:
        ...
        return tuple(
            ...
            if scope.function_id == function_id and scope.pc_range.contains(frame_pc)
        )

    def variables_in_scope(
        self, lexical_scope_id: str, frame_pc: HsxAddress
    ) -> tuple[SymbolRecord, ...]:
        ...
        if not isinstance(frame_pc, HsxAddress):
            raise TypeError("frame_pc must be HsxAddress")
        scope = next(
            (record for record in self._scopes if record.lexical_scope_id == lexical_scope_id),
            None,
        )
        ...
        return tuple(
            ...
            for symbol in sorted(
                self._symbols, key=lambda record: (record.declaration_order, record.symbol_id)
            )
            if symbol.lexical_scope_id == lexical_scope_id
            and symbol.kind in {SymbolKind.LOCAL, SymbolKind.CONSTANT}
        )
        ...
        return tuple(
            sorted(
                (
                    ...
                    or (
                        symbol.kind is SymbolKind.CONSTANT
                        and symbol.function_id is None
                        and symbol.lexical_scope_id is None
                    )
                ),
                key=lambda record: (record.declaration_order, record.symbol_id),
            )
        )
    ...
    def symbols_named(self, name: str) -> ResolutionResult[SymbolRecord]:
        ...
        return _query_result(
            self._binding,
            values,
            multiple=ResolutionStatus.AMBIGUOUS,
            unavailable_code="symbol_unavailable",
    ...
    def instruction_at(self, address: HsxAddress) -> ResolutionResult[InstructionRecord]:
        ...
        return _query_result(
            self._binding,
            values,
            unavailable_code="instruction_unavailable",
            multiple_code="instruction_identity_overlap",
        )

    def source_locations(
        self, source: SourceRef, line: int, column: int | None
    ) -> ResolutionResult[InstructionRecord]:
        ...
        values = tuple(
            ...
            and record.source.line == line
            and (column is None or record.source.column == column)
        )
        if not values:
            return ResolutionResult(
                ResolutionStatus.UNAVAILABLE,
                self._binding,
                (),
                (Diagnostic("source_location_unavailable", "no executable source location matched", component="artifact_index"),),
            )
    ...
    def unwind_rows(
        self, pc: HsxAddress, function_id: str | None
    ) -> ResolutionResult[UnwindRow]:
        ...
        if function_id is not None:
            functions = tuple(
                ...
                if function.function_id == function_id and function.range.contains(pc)
            )
            if len(functions) != 1:
                return _query_result(
                    self._binding,
                    (),
                    unavailable_code="unwind_function_unavailable",
                    multiple_code="unwind_function_overlap",
                )
        values = tuple(row for row in self._unwind_rows if row.pc_range.contains(pc))
        return _query_result(
            self._binding,
            values,
            unavailable_code="unwind_row_unavailable",
            multiple_code="unwind_row_overlap",
        )
    ...
    def location_rows(
        ...
        values = tuple(
            ...
            and row.function_id == function_id
            and row.lexical_scope_id == lexical_scope_id
            and row.pc_range.contains(frame_pc)
        )
        return _query_result(
            self._binding,
            values,
            unavailable_code="location_row_unavailable",
            multiple_code="location_row_overlap",
        )
