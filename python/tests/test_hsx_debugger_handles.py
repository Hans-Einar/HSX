...
def test_intern_is_exact_idempotent_and_monotonic() -> None:
    ...
    assert (first.value.serial, second.value.serial) == (1, 2)
    assert store.resolve(first.value, HandleKind.FRAME).object_key == (0,)


def test_scope_and_variable_key_shapes_are_frozen() -> None:
    ctx = context()
    store = EpochHandleStore.create(ctx)
    frame = store.intern(HandleKind.FRAME, (0,)).value
    scope = store.intern(HandleKind.SCOPE, (frame.serial, ScopeKind.LOCALS)).value
    symbol = store.intern(HandleKind.VARIABLE, (scope.serial, "symbol", 4, "local.x")).value
    register = store.intern(HandleKind.VARIABLE, (scope.serial, "register", 2, "R7")).value
...
def test_invalidate_is_terminal_and_idempotent() -> None:
    ...
    assert store.intern(HandleKind.FRAME, (1,)).status is InspectionStatus.STALE
...
def test_concurrent_same_key_interns_one_serial() -> None:
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        handles = tuple(pool.map(lambda _: store.intern(HandleKind.FRAME, (7,)).value, range(64)))
...
def test_concurrent_distinct_keys_get_unique_never_reused_serials() -> None:
    ...
    store = EpochHandleStore.create(ctx)
    with ThreadPoolExecutor(max_workers=8) as pool:
        handles = tuple(pool.map(lambda i: store.intern(HandleKind.FRAME, (i,)).value, range(32)))
