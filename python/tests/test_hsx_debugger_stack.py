...
def foundation() -> SimpleNamespace:
    artifact = ArtifactRef("hsx.artifact-ref/1", "application/vnd.hsx.hxe", CanonicalUInt64(1), CanonicalUInt64(64), ContentDigest("sha256", ZERO))
    target = TargetRef("target", ExecutiveInstanceRef("exec"), "opaque", 1, 7, 1)
    image = LoadedImageRef("hsx.loaded-image-ref/1", target.executive, target, "image", CanonicalUInt64(1), artifact)
    ...
    bundle_ref = ImageDebugBundleRef("hsx.image-debug-bundle-ref/1", artifact, "sha256", bundle.canonical_digest())
    binding_payload = ImageDebugBindingPayload(
        "hsx.image-debug-binding/1", image, bundle_ref,
        "hsx.arch.vm-byte16-gpr32/1", "hsx.abi.llc-r7-word32/1", "hsx.portable-debug-runtime/1"
    )
    ...
    architecture = ArchitectureDescriptor(
        ...
        (
            AddressSpaceDescriptor(code, "byte", 8, 16, (HsxAddressRange(HsxAddress(code, 0), 0x1000),), ByteOrder.LITTLE, 4, WrapPolicy.FORBIDDEN, frozenset({Permission.READ, Permission.EXECUTE})),
            AddressSpaceDescriptor(data, "byte", 8, 16, (HsxAddressRange(HsxAddress(data, 0), 0x1000),), ByteOrder.LITTLE, 1, WrapPolicy.FORBIDDEN, frozenset({Permission.READ, Permission.WRITE})),
        ),
    ...
    snapshot = InspectionSnapshotRef(target, image, stop, "snapshot", 3, 4, frozenset({"registers", "memory", "disassembly"}), SnapshotStability.IMMUTABLE, EvidenceGrade.PORTABLE)
    context = InspectionContext(target, image, EpochBinding(StopEpochId("epoch"), generation, stop, snapshot, EvidenceGrade.PORTABLE))
    registers = RegisterSet((
        RegisterValue("R0", 32, 1, True), RegisterValue("R1", 32, 2, True),
        RegisterValue("R7", 32, 0x200, True), RegisterValue("PC", 16, 0x100, True),
        RegisterValue("SP", 16, 0x1E0, True), RegisterValue("PSW", 8, 0xA5, True),
    ))
    ...
    body = UnwindRow(
        "body", binding, HsxAddressRange(HsxAddress(code, 0x100), 4), abi, schema,
        expr(
            RecipeRole.CFA,
            (RegValueOp("reg_value", "R7"), BitSliceOp("bit_slice", 0, 32), ToAddressOp("to_address", data), AddSConstCheckedOp("add_sconst_checked", 8)),
            RecipeResultKind.ADDRESS, None
        ),
        RecipeRule(
            RecipeRuleKind.EXPRESSION,
            expr(RecipeRole.CALLER_PC, (CfaOp("cfa"), AddSConstCheckedOp("add_sconst_checked", -4), DerefUOp("deref_u", 4, ByteOrder.LITTLE), ToAddressOp("to_address", code)), RecipeResultKind.ADDRESS, None),
            None
        ),
        RecipeRule(RecipeRuleKind.EXPRESSION, expr(RecipeRole.CALLER_SP, (CfaOp("cfa"),), RecipeResultKind.ADDRESS, None), None),
        None,
        (("R7", RecipeRule(RecipeRuleKind.EXPRESSION, expr(RecipeRole.REGISTER, (CfaOp("cfa"), AddSConstCheckedOp("add_sconst_checked", -8), DerefUOp("deref_u", 4, ByteOrder.LITTLE)), RecipeResultKind.UNSIGNED_SCALAR, 32), None)),),
        UnwindBoundary.ORDINARY, -4
    )
    terminal = UnwindRow(
        "terminal", binding, HsxAddressRange(HsxAddress(code, 0x120), 8), abi, schema,
        expr(RecipeRole.CFA, (SpecialValueOp("special_value", RecipeSpecial.SP), AddSConstCheckedOp("add_sconst_checked", 4)), RecipeResultKind.ADDRESS, None),
        RecipeRule(RecipeRuleKind.UNAVAILABLE, None, "terminal"),
        RecipeRule(RecipeRuleKind.UNAVAILABLE, None, "terminal"),
        None, (), UnwindBoundary.TERMINAL, None
    )
...
class IndexDouble:
    def __init__(self, f, rows=None, instructions=None, binding=None):
        ...
        self.instructions = dict(instructions if instructions is not None else {0x120: f.instruction})
        self._binding = binding or f.binding

    def binding(self): return self._binding
    def bundle_identity(self): return self.f.bundle
    def functions(self): return (self.f.function0, self.f.function1)
    def instruction_at(self, address):
        value = self.instructions.get(address.unsigned_value)
        if value is None:
            return ResolutionResult(ResolutionStatus.UNAVAILABLE, self._binding, (), (Diagnostic("instruction_unavailable", "missing", component="test"),))
    ...
    def unwind_rows(self, pc, function_id):
        ...
        if not matches:
            return ResolutionResult(ResolutionStatus.UNAVAILABLE, self._binding, (), (Diagnostic("unwind_row_unavailable", "missing", component="test"),))
        return ResolutionResult(ResolutionStatus.CORRUPT, self._binding, (), (Diagnostic("unwind_row_overlap", "overlap", component="test"),))
...
class SnapshotPort:
    ...
    def read_memory(self, context, address, byte_length):
        ...
        return InspectionResult(InspectionStatus.COMPLETE, context, MemoryBlock(address, byte_length, (MemorySegment(0, byte_length, raw, MemorySegmentStatus.COMPLETE),)), ())
...
def unwind(f, *, index=None, port=None, request=None):
    return StackService.unwind(f.context, index or IndexDouble(f), port or SnapshotPort(f), f.architecture, f.abi, RecipeLimits(), request or RecipeRequestLimits(64, 16))
...
def test_stack_requires_register_read_set_before_touching_snapshot_port() -> None:
    ...
    f.context = replace(
        f.context,
        epoch=replace(f.context.epoch, snapshot=snapshot),
    )
    port = SnapshotPort(f)
