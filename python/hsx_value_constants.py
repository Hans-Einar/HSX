...
"""Load shared HSX value constants from the C header.
...
def load_value_constants() -> Dict[str, Union[int, str]]:
    """Parse `hsx_value.h` and return a name->value mapping."""
    if not _HEADER_PATH.exists():
        raise FileNotFoundError(f"missing value header: {_HEADER_PATH}")

    constants: Dict[str, Union[int, str]] = {}
    for raw_line in _HEADER_PATH.read_text(encoding="utf-8").splitlines():
        line = raw_line.strip()
        if not line or not line.startswith("#define HSX_VAL_"):
            continue
...
CONSTANTS = load_value_constants()
