...
"""Load shared HSX command constants from the C header.
...
_HEADER_BASENAME = "hsx_command.h"
...
def load_command_constants() -> Dict[str, Union[int, str]]:
    """Parse `hsx_command.h` and return a name->value mapping."""
    if not _HEADER_PATH.exists():
        raise FileNotFoundError(f"missing command header: {_HEADER_PATH}")

    constants: Dict[str, Union[int, str]] = {}
    for raw_line in _HEADER_PATH.read_text(encoding="utf-8").splitlines():
        line = raw_line.strip()
        if not line or not line.startswith("#define HSX_CMD_"):
            continue
...
CONSTANTS = load_command_constants()
